    ...
    private String getGenotypeByShort (short[] cnt) {   // is for a single taxon
        ...
        for (int i = 0; i < cnt.length; i++) {  // every allele
            for (int j = i; j < cnt.length; j++) {  // another allele (including itself)
                ...
                if (i == j) {
                    value = -Math.log10(coe*Math.pow((1-0.75*this.combinedErrorRate), cnt[i])*Math.pow(this.combinedErrorRate /4, (sum-cnt[i])));   // combinedErrorRate is the wrongly sequencing rate
